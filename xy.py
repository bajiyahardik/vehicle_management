...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        # Create Frames
        input_frame = tk.Frame(self, bg='#dcdcdc')
        ...
        # Vehicle ID
        self.lbl_vehicle_id = tk.Label(input_frame, text="Vehicle ID:", bg='#dcdcdc', font=('Helvetica', 10))
        self.lbl_vehicle_id.grid(row=0, column=0, padx=10, pady=5)
        self.entry_vehicle_id = tk.Entry(input_frame, font=('Helvetica', 10))
        self.entry_vehicle_id.grid(row=0, column=1, padx=10, pady=5)

        # Make
        self.lbl_make = tk.Label(input_frame, text="Make:", bg='#dcdcdc', font=('Helvetica', 10))
        self.lbl_make.grid(row=1, column=0, padx=10, pady=5)
        self.entry_make = tk.Entry(input_frame, font=('Helvetica', 10))
        self.entry_make.grid(row=1, column=1, padx=10, pady=5)

        # Model
        self.lbl_model = tk.Label(input_frame, text="Model:", bg='#dcdcdc', font=('Helvetica', 10))
        self.lbl_model.grid(row=2, column=0, padx=10, pady=5)
        self.entry_model = tk.Entry(input_frame, font=('Helvetica', 10))
        self.entry_model.grid(row=2, column=1, padx=10, pady=5)

        # Year
        self.lbl_year = tk.Label(input_frame, text="Year:", bg='#dcdcdc', font=('Helvetica', 10))
        self.lbl_year.grid(row=3, column=0, padx=10, pady=5)
        self.entry_year = tk.Entry(input_frame, font=('Helvetica', 10))
        self.entry_year.grid(row=3, column=1, padx=10, pady=5)

        # Color
        self.lbl_color = tk.Label(input_frame, text="Color:", bg='#dcdcdc', font=('Helvetica', 10))
        self.lbl_color.grid(row=4, column=0, padx=10, pady=5)
        self.entry_color = tk.Entry(input_frame, font=('Helvetica', 10))
        self.entry_color.grid(row=4, column=1, padx=10, pady=5)

        # Buttons
        self.btn_add = tk.Button(input_frame, text="Add Vehicle", command=self.add_vehicle, bg='#4CAF50', fg='white', font=('Helvetica', 10))
        self.btn_add.grid(row=5, column=0, padx=10, pady=5)

        self.btn_view = tk.Button(input_frame, text="View Vehicle", command=self.view_vehicle, bg='#2196F3', fg='white', font=('Helvetica', 10))
        self.btn_view.grid(row=5, column=1, padx=10, pady=5)

        self.btn_update = tk.Button(input_frame, text="Update Vehicle", command=self.update_vehicle, bg='#FF9800', fg='white', font=('Helvetica', 10))
        self.btn_update.grid(row=6, column=0, padx=10, pady=5)

        self.btn_delete = tk.Button(input_frame, text="Delete Vehicle", command=self.delete_vehicle, bg='#F44336', fg='white', font=('Helvetica', 10))
        self.btn_delete.grid(row=6, column=1, padx=10, pady=5)

        self.btn_list = tk.Button(input_frame, text="List All Vehicles", command=self.list_vehicles, bg='#9C27B0', fg='white', font=('Helvetica', 10))
        self.btn_list.grid(row=7, column=0, columnspan=2, padx=10, pady=5)

        # List Box to show vehicles
        self.list_box = tk.Listbox(list_frame, width=80, height=15, font=('Helvetica', 10))
        self.list_box.pack()
